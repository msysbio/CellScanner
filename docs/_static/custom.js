...
const RENDER_PIPELINES = ["birp", "hdrp", "urp"];
    ...
    $("article[role='main']").prepend(`
            <div class="admonition attention">
                ...
                <p>
                    ...
                    or compatible with the latest <em>CellScanner</em>.
                </p>
                <p class="last">
...
if (typeof isPage404 !== "undefined" && isPage404 && isVersion) {
  $.ajax({
    ...
    error: (_) => {
      ...
      if (!newUrl.pathname.endsWith("/") && !newUrl.pathname.endsWith(".html"))
        newUrl.pathname += "/";
...
if (window.matchMedia) {
  ...
  $(document).ready((_) =>
    applyLightOrDarkMode(
      window.matchMedia("(prefers-color-scheme: dark)").matches
    )
  );
  window
    .matchMedia("(prefers-color-scheme: dark)")
    .addEventListener("change", (event) => applyLightOrDarkMode(event.matches));
}
